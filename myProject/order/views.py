...
def addfavorite(request,id):
    ...
    checkinproduct =AddFavourite.objects.filter(product_id=id,user_id=current_user.id)
    if checkinproduct:
        ...
        request.session['favori_items'] = AddFavourite.objects.filter(user_id = current_user.id).count()
...
def favorites(request):
    current_user = request.user
    favorites =AddFavourite.objects.filter(user=current_user)
...
def delfavorite(request,id):
    url = request.META.get('HTTP_REFERER')
    current_user = request.user
    AddFavourite.objects.filter(product_id=id, user_id=current_user.id).delete()
    return HttpResponseRedirect(url)
